...
import {
  ShoppingCart, LogOut, Moon, Sun, Trash2, Plus, AlertCircle, CheckCircle, Menu, X, Home, Settings, BarChart3, User as UserIcon
} from 'lucide-react';
...
export default function CashierPage() {
  ...
  return (
    <div className={`min-h-screen w-full transition-colors duration-300 ${
      ...
      {/* Header */}
      <header className={`border-b transition-colors sticky top-0 z-40 w-full ${
        ...
        <div className="px-3 sm:px-6 lg:px-8 py-3 sm:py-4 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg sm:text-2xl font-bold hidden sm:inline">RetailOS</h1>
            </div>
          </div>
        ...
        <div className="sm:hidden px-3 pb-3 flex gap-1 overflow-x-auto border-t" style={{
      ...
      <div className="flex flex-col sm:flex-row w-full">
        ...
        {sidebarOpen && (
          <>
            ...
            />
            <aside className={`fixed left-0 top-32 h-screen w-64 border-r transition-colors z-30 overflow-y-auto sm:static sm:top-0 sm:h-auto sm:border-r ${
        ...
        <main className="flex-1 p-3 sm:p-8 pb-24 sm:pb-8 w-full">
          ...
          {activeTab === "pos" && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-6">
              <div className="lg:col-span-2 space-y-3 sm:space-y-6">
                {message && (
                  <div className={`p-3 sm:p-4 rounded-lg border flex items-center gap-2 sm:gap-3 text-sm ${
                    messageType === "success"
                      ? isDark
                ...
                {/* Scanner */}
                <div className={`rounded-lg sm:rounded-2xl border p-3 sm:p-6 ${
                  isDark
                    ? 'bg-slate-900 border-slate-800'
                    : 'bg-white border-slate-200'
                }`}>
                  <div className="flex justify-between items-center mb-3 sm:mb-4">
                    <h2 className="text-base sm:text-xl font-bold">Barcode Scanner</h2>
                    <button
                ...
                <div className={`rounded-lg sm:rounded-2xl border p-3 sm:p-6 ${
                  ...
                  <h2 className="text-base sm:text-xl font-bold mb-3">Manual Entry</h2>
              ...
              {/* Cart */}
              <div className={`rounded-lg sm:rounded-2xl border p-3 sm:p-6 h-fit lg:sticky lg:top-24 ${
                isDark
                  ? 'bg-slate-900 border-slate-800'
                  : 'bg-white border-slate-200'
              }`}>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-base sm:text-xl font-bold">Cart ({cart.length})</h2>
                </div>

                {cart.length === 0 ? (
                  <p className={`text-center py-6 text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                    No items
                  </p>
                ) : (
                  <>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs font-semibold ml-1">
                              Rs. {(item.quantity * (item.price || 0)).toFixed(0)}
                            </span>
                          </div>
                        </div>
                      ))}
          ...
          {/* Sales Tab */}
          {activeTab === "sales" && (
            <div className={`rounded-lg sm:rounded-2xl border p-3 sm:p-8 ${
              isDark
                ? 'bg-slate-900 border-slate-800'
                : 'bg-white border-slate-200'
            }`}>
              <h2 className="text-xl sm:text-2xl font-bold mb-4">Sales History</h2>
              {sales.length === 0 ? (
                <p className={`text-center py-8 text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  No sales yet
                </p>
              ) : (
                <div className="space-y-3">
                  {sales.map((sale) => (
                    <div key={sale.id} className={`p-3 sm:p-4 rounded-lg border ${
                      isDark
                        ? 'bg-slate-800 border-slate-700'
                        : 'bg-slate-50 border-slate-200'
                    }`}>
                      <div className="flex justify-between items-start gap-2 mb-2">
                        <div className="min-w-0">
                          <p className="font-semibold text-sm">Sale #{sale.id.slice(0, 8).toUpperCase()}</p>
                          <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                            {new Date(sale.timestamp).toLocaleString()}
                          </p>
                        </div>
                        <p className="text-base sm:text-lg font-bold text-cyan-500 flex-shrink-0">Rs. {sale.total.toFixed(2)}</p>
                      </div>
                      <div className="space-y-1 text-xs">
                        {sale.items.map((item, idx) => (
                          <div key={idx} className="flex justify-between gap-2">
                            <span className="truncate">{item.name} x{item.quantity}</span>
                            <span className="flex-shrink-0">Rs. {(item.quantity * item.price).toFixed(2)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {/* Settings Tab */}
          {activeTab === "settings" && (
            <div className={`rounded-lg sm:rounded-2xl border p-3 sm:p-8 w-full max-w-2xl mx-auto ${
              isDark
                ? 'bg-slate-900 border-slate-800'
                : 'bg-white border-slate-200'
            }`}>
              <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Profile Settings</h2>

              {settingsMsg && (
                <div className={`mb-4 sm:mb-6 p-3 sm:p-4 rounded-lg border flex items-center gap-2 text-sm ${
                  ...
                  {settingsType === "success" ? <CheckCircle size={16} /> : <AlertCircle size={16} />}
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="border-t pt-4 sm:pt-6" style={{ borderTopColor: isDark ? '#334155' : '#e2e8f0' }}>
